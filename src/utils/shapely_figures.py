...
def plot_line(ax, ob, color=GRAY, zorder=1, linewidth=LINE_WIDTH, alpha=1):
    x, y = ob.xy
    ax.plot(
        x,
        y,
...
def plot_poly(ax, ob, color=GRAY, zorder=1, alpha=1):
    x, y = ob.exterior.xy
    ax.fill(x, y, color=color, zorder=zorder, alpha=alpha)

def plot_coords(ax, ob, color=GRAY, zorder=1, alpha=1):
    x, y = ob.xy
    ax.plot(x, y, "o", markersize=2, color=color, zorder=zorder, alpha=alpha)
